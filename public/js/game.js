  ...
  try{
    ...
    if(qp.get("safe")==="1") window.SAFE_MODE = true;

    const mtn = (qp.get("mtn")||"").toLowerCase();
    if(mtn === "trial") window.DEBUG_MOUNTAIN_MODE = "TRIAL";
    if(mtn === "final") window.DEBUG_MOUNTAIN_MODE = "FINAL";

    const steel = (qp.get("steel")||"").toLowerCase();
    if(steel === "hunk" || steel === "4") window.DEBUG_STEEL_MODE = "HUNK";
    if(steel === "bro" || steel === "older" || steel === "brother" || steel === "13") window.DEBUG_STEEL_MODE = "BRO";

    // --- Start-depth TEST HARNESS (query-string only) ---
    // ?start=19  -> begin on level 19 so level 20 is one descent away
    // ?start=20  -> begin directly on level 20
    window.DEBUG_START_DEPTH = null;
    const startRaw = (qp.get("start") || qp.get("lvl") || "").toLowerCase();
    if(startRaw){
      const n = parseInt(startRaw, 10);
      if(Number.isFinite(n) && n > 0) window.DEBUG_START_DEPTH = n;
    }

    // --- Near-Death TEST HARNESS (query-string only) ---
    // ?nd=force  -> force on next defeat (any depth)
    // ?nd=12     -> force on defeat only at depth 12
    // ?nd=1      -> armed (but not forced unless you add chance)
    // Optional: &ndOnce=1 -> disable after first trigger
    const ndRaw = (qp.get("nd") || "").toLowerCase();

    window.TEST_NEAR_DEATH = false;
    window.TEST_NEAR_DEATH_FORCE = false;
    window.TEST_NEAR_DEATH_DEPTH = null;
    window.TEST_NEAR_DEATH_ONCE = (qp.get("ndOnce") === "1");

    if(ndRaw){
      window.TEST_NEAR_DEATH = true;
      if(ndRaw === "force") {
        window.TEST_NEAR_DEATH_FORCE = true;
      } else if(ndRaw === "1" || ndRaw === "true" || ndRaw === "on") {
        // armed but not forced
      } else {
        const n = parseInt(ndRaw, 10);
        if(Number.isFinite(n) && n > 0) window.TEST_NEAR_DEATH_DEPTH = n;
      }
    }
  }catch(e){}

  function shouldForceNearDeathNow(){
    if(!window.TEST_NEAR_DEATH) return false;
    if(window.TEST_NEAR_DEATH_FORCE) return true;
    if(Number.isFinite(window.TEST_NEAR_DEATH_DEPTH)){
      return window.game && window.game.depth === window.TEST_NEAR_DEATH_DEPTH;
    }
    return false;
  }

  function consumeNearDeathHarnessIfNeeded(){
    if(window.TEST_NEAR_DEATH_ONCE){
      window.TEST_NEAR_DEATH = false;
      window.TEST_NEAR_DEATH_FORCE = false;
      window.TEST_NEAR_DEATH_DEPTH = null;
    }
...
(() => {
  ...
  // ---------- NEAR-DEATH CHANCE (normal play) ----------
  // Many players will never see Near-Death
  // Some will see it once and remember it
  // Almost no one will “expect” it
  const NEAR_DEATH_CHANCE = 0.05; // 0.02 = 2% chance on defeat (0.00–1.00)


  // DEBUG Mountain test harness:
  // In real play: Mountain appears on depth 10 (TRIAL) and depth 20 (FINAL).
  // In debug: you can force a level-1 Mountain encounter that mimics either:
  //   window.DEBUG_MOUNTAIN_MODE = "TRIAL"  // level-10 behavior
  //   window.DEBUG_MOUNTAIN_MODE = "FINAL"  // level-20 behavior
  // Set to null for normal behavior.
  if(typeof window.DEBUG_MOUNTAIN_MODE !== "string") window.DEBUG_MOUNTAIN_MODE = null;
  // Debug harness is opt-in via URL (?mtn=trial or ?mtn=final). No default.

  function getMountainMode(depth){
    // Debug harness: depth 1 can impersonate TRIAL or FINAL.
    if(window.DEBUG && depth === 1 && (window.DEBUG_MOUNTAIN_MODE === "TRIAL" || window.DEBUG_MOUNTAIN_MODE === "FINAL")){
      return window.DEBUG_MOUNTAIN_MODE;
    }
    // Real game
    if(depth === 10) return "TRIAL";
    if(depth === 20) return "FINAL";
    return null;
  }

  // DEBUG Steel test harness:
  // In real play: Steel "hunk" appears on depth 4, and its older brother appears on depth 13.
  // In debug: you can force a level-1 Steel encounter via URL:
  //   ?debug=1&steel=hunk   (depth-4 behavior)
  //   ?debug=1&steel=bro    (depth-13 behavior)
  // Set to null for normal behavior.
  if(typeof window.DEBUG_STEEL_MODE !== "string") window.DEBUG_STEEL_MODE = null;

  function getSteelMode(depth){
    // Debug harness: depth 1 can impersonate either steel encounter.
    if(window.DEBUG && depth === 1 && (window.DEBUG_STEEL_MODE === "HUNK" || window.DEBUG_STEEL_MODE === "BRO")){
      return window.DEBUG_STEEL_MODE;
    }
    // Real game
    if(depth === 4) return "HUNK";
    if(depth === 13) return "BRO";
    return null;
  }

  const DEBUG_START_DEPTH = clampDebugStartDepth(window.DEBUG_START_DEPTH);
  function clampDebugStartDepth(n){
    const v = parseInt(n, 10);
    if(!Number.isFinite(v)) return null;
    return Math.max(1, Math.min(20, v));
  }

  // Steel encounter: immobile, low damage, meant as foreshadowing (not a minion swarm).
  function spawnSteelEncounter(lvl, depth, mode){
    if(!lvl || !lvl.tiles || !lvl.monsters) return;

    const p = lvl.player || {x:2,y:2};
    const isBro = (mode === "BRO");
    const glyph = isBro ? ENT.CUBE : ENT.ORC; // underlying glyphs; renderer swaps to ◉ / ◎
    const name  = isBro ? "Heap of Metal's Older Brother" : "Heap of Metal With Eyes";

    const hpMax = isBro ? 24 : 12;
    const atk   = isBro ? 6  : 3;

    // Find a place. In DEBUG depth-1 harness, spawn near the player so it's immediately testable.
    let bx = null, by = null;

    if(window.DEBUG && depth === 1){
      // Prefer a reachable/visible tile near the player. We expand a radius ring until we find a free floor/corr.
      const MIN_R = 2;
      const MAX_R = 24;
      for(let r=MIN_R; r<=MAX_R && bx===null; r++){
        // Sample a bunch of points on the ring (cheap, avoids full scan).
        for(let tries=0; tries<80 && bx===null; tries++){
          const dx = randInt(-r, r);
          const dy = (Math.random()<0.5 ? -1 : 1) * (r - Math.abs(dx));
          const x = p.x + dx;
          const y = p.y + dy;
          ...
          const k = keyXY(x,y);
          if(lvl.monsters.has(k) || lvl.traders?.has(k)) continue;
          bx = x; by = y;
        }
      }
    }

    // Fallback: pick a random floor/corr tile away from the player.
    if(bx === null){
      for(let tries=0; tries<9000 && bx===null; tries++){
        const x = randInt(2, MAP_W-3);
        const y = randInt(2, MAP_H-3);
        const t = lvl.tiles[x][y];
        if(!(t===TILE.FLOOR || t===TILE.CORR)) continue;
        if(Math.abs(x-p.x) <= 10 && Math.abs(y-p.y) <= 6) continue; // keep it off the doorstep
        const k = keyXY(x,y);
        if(lvl.monsters.has(k) || lvl.traders?.has(k)) continue;
        bx = x; by = y;
      }
    ...
    // Last resort: place it a bit to the right of the player, even if that means it may land in a corridor.
    if(bx===null){ bx = Math.min(MAP_W-3, p.x+8); by = p.y; }
    ...
    const m = {
      ...
      steelKind: mode,
      steelStage: 0,
      color: isBro ? "#d6dde7" : "#a9b4c3"
    };
    lvl.monsters.set(keyXY(bx,by), m);

    try{ if(window.DEBUG && depth===1) console.log('[STEEL HARNESS]', mode, 'spawned at', bx, by); }catch(e){}

    // One-time intro beat (lore tone; not too meta).
    if(mode === "HUNK"){
      logMsg("A crude mass of steel rises from the floor tiles.", "lore");
    }else{
      logMsg("Something heavy answers your footsteps.", "lore");
    }
  }
...
// Torches spawn “fairly liberally”
  ...
  function syncModes(){
    // DEBUG and SAFE_MODE are independent. SAFE_MODE is opt-in (e.g., ?safe=1).
    if(typeof window.SAFE_MODE !== "boolean") window.SAFE_MODE = false;
  }
...
const ENT = {
  ...
  const MONSTER_COLOR = {
    ...
    [ENT.SKELETON]: '#e7e7e7',
    [ENT.HORROR]: COL.boss,
    [ENT.MOUNTAIN]: COL.boss,
    ['▓']: '#ffb8a1',
    ['▒']: '#ffd4c7',
    ['░']: '#fff0ea',
    [ENT.MOUNTAIN_SHARD_A]: '#d9c6a5',
    [ENT.MOUNTAIN_SHARD_B]: '#d9c6a5',
     ['╬']: '#cfd8dc',
     ['^']: '#b0bec5',
     ['◎']: '#a9b4c3',
     ['◉']: '#d6dde7'
  ...
  const SCROLL_GLYPHS = ["☼","▲","Ψ","¤"];
  const SCROLL_NAME = { "☼":"Illumination", "▲":"Grounding", "Ψ":"Attunement", "¤":"Diminution" };
  const SCROLL_COLOR = { "☼":"#ffe9a6", "▲":"#d7c9a1", "Ψ":"#b7f0ff", "¤":"#e3c8ff" };
  const SCROLL_CAP = 12;
  ...
  const GLYPH_REGISTRY = [
    ...
    { glyph: ENT.MOUNTAIN, name: "Mountain", color: MONSTER_COLOR[ENT.MOUNTAIN] },
    { glyph: ENT.MOUNTAIN_SHARD_A, name: "Mountain Shard", color: MONSTER_COLOR[ENT.MOUNTAIN_SHARD_A] }
  ];
...
function renderGlyphCanvas(){
    ...
    for(let y=0; y<frame.rows; y++){
      for(let x=0; x<frame.cols; x++){
        ...
        // --- STEEL RENDER OVERRIDE (boss-only, keeps underlying glyphs as o/O) ---
        // IMPORTANT: render-only, read-only. Use loop indices x/y + viewX/viewY (cell.x/y is not guaranteed).
        try{
          const __lvlS = (window.game && typeof getLevel === 'function') ? getLevel(game.depth) : null;
          if(__lvlS && __lvlS.monsters && __lvlS.monsters.get){
            const __vx = (typeof viewX !== "undefined" ? viewX : 0);
            const __vy = (typeof viewY !== "undefined" ? viewY : 0);
            const __wx2 = __vx + x;
            const __wy2 = __vy + y;
            const __mk = keyXY(__wx2, __wy2);
            const __m2 = __lvlS.monsters.get(__mk) || null;
            if(__m2 && __m2.steelKind){
              ch = (__m2.steelKind === "BRO") ? "◉" : "◎";
              glyphColor = (__m2.color || glyphColor);
            }
          }
        }catch(_){ }
...
let __skipCell = false
        ...
        if(__anim){
          ...
          if(__anim.type === "door_open"){
            ...
            // obvious: jitter + flash between X and ✖
            const jit = Math.round(Math.sin(uAnim * Math.PI * 10) * 2);
            __ox = jit;
            __oy = 0;
            ch = (Math.sin(uAnim * Math.PI * 6) > 0) ? "X" : "✖";
          }
}
        ...
        if(!__cellDone && ch !== " " ){
          ...
          // (Scroll reserved for future: '?' or '§' if you add them later)
          const isPotionGlyph = (ch === ENT.POTION);
          const isKeyGlyph = (ch === "k");
          const isScrollGlyph = (ch === "☼" || ch === "▲" || ch === "Ψ" || ch === "¤");
...
// Weapon item shimmer (single-tile, contained): *slow* metallic sweep + obvious glyph shine
          if(ch === ENT.WEAPON){
            ...
            if(sweep > 0.86){
              ...
              gctx.fillStyle = "rgba(255,255,255,1)";
              gctx.fillText("✦", drawX + __ox + charW*0.66, drawY + __oy + charH*0.40);
            }
          ...
          if(__anim && __anim.type === "spark"){
            ...
            gctx.fillStyle = "rgba(255,255,255,1)";
            gctx.fillText("✶", drawX + __ox, drawY + __oy);
gctx.restore();
          }
...
// NOTE: This block is renderer-neutral and safe.
// We rely on <base id="ddBase"> (set in <head>) so relative URLs work on both:
// - file:// (base="") => "images/..." resolves to sibling folder
// - http(s):// (base="/") => "images/..." resolves to "/images/..."
const ASSET = {
  img:   "images/",
  music: "music/",
  sfx:   "sounds/"
...
// Prefix bare filenames OR subpaths (e.g. "soundtrack/..") unless already rooted.
function resolveAsset(kind, src){
  ...
  // Explicitly rooted / already relative
  if(s.startsWith("/") || s.startsWith("./") || s.startsWith("../")) return s;

  // Already kind-rooted (avoid double-prefix)
  if(kind === "img"   && s.startsWith("images/")) return s;
  if(kind === "music" && s.startsWith("music/"))  return s;
  if(kind === "sfx"   && s.startsWith("sounds/")) return s;
...
try{ $bgm.pause(); }catch(e){}
  ...
  function onEnterCaves() {
    ...
    if((game.loreWeight|0) >= RECOG_THRESHOLD){
      fireLoreOnce('cave_enter_recog', 'Stone closes behind you—remembering the shape of your passing.');
    } else {
  ...
  function resumeFallback(){
    ...
    // Prefer roam pool, but NEVER pick the base theme as fallback.
    const pool = (Music.pools && Music.pools.roam) ? Music.pools.roam : [];
    let pickSrc = null;

    if(pool.length){
      // Try a few times to avoid landing on base theme
      for(let i = 0; i < 12; i++){
        const p = weightedPick(pool);
        const s = p && p.src;
        if(s && s !== (Music.base && Music.base.src)){
          pickSrc = s;
          break;
        }
      }
      // If everything is base (or bad), fall back to the first non-base we can find
      if(!pickSrc){
        const alt = pool.find(it => it && it.src && it.src !== (Music.base && Music.base.src));
        pickSrc = alt ? alt.src : null;
      }
    }

    if(pickSrc){
      Music.state.mode = "clip"; // treated like a normal clip/track
      playClip(pickSrc);
      return;
    }

    // Absolute last resort: if no roam pool exists, keep old behavior
    resumeBase();
  }
...
setMuted(uiMuted);
  ...
  const SFX_MASTER = 4.0; // global SFX loudness (0..1)

  const SFX_DEFS = {
    // Movement
    step:             { src: "sfx_step.wav",                 vol: 0.28 }, // was a touch hot
    step_corpsefield: { src: "sfx_step_corpsefield.wav",     vol: 0.45 },


    move_water:       { src: "sfx_move_water.wav",          vol: 0.55 },
    move_mist:        { src: "sfx_move_mist.wav",           vol: 0.55 },
    move_fire:        { src: "sfx_move_fire.wav",           vol: 0.60 },
    move_slime:       { src: "sfx_move_slime.wav",          vol: 0.55 },

    // Core interactions
    bump:     { src: "sfx_wall_bump.wav",            vol: 0.50 },
    locked_bump:{ src:"sfx_locked_bump.wav",         vol: 0.45 },

    // Pickups (semantic)
    pickup:        { src: "sfx_pickup.wav",          vol: 0.50 }, // fallback / generic
    pickup_chest:  { src: "sfx_pickup_chest.wav",    vol: 0.70 },
    pickup_potion: { src: "sfx_pickup_potion.wav",   vol: 0.55 },
    pickup_scroll: { src: "sfx_pickup_scroll.wav",   vol: 0.55 },
    pickup_key:    { src:'sfx_pickup.wav', vol:0.95 },
    pickup_weapon: { src:'sfx_pickup.wav', vol:0.95 },

    pickup_relic:  { src: "sfx_pickup_relic.wav",    vol: 0.80 },
    pickup_shrine: { src: "sfx_pickup_shrine.wav",   vol: 0.80 },

    torch_pickup:   { src: "sfx_torch_pickup.wav",          vol: 0.45 },
    torch_light:    { src: "sfx_torch_light.wav",           vol: 0.55 },
    trap_trigger:   { src: "sfx_trap_trigger.wav",       vol: 0.60, fallback: ["sfx_damage_trap.wav"] },
    encounter_bats:{ src: "sfx_encounter_bats.wav",       vol: 0.55 },

    // Consumables / combat
    drink:    { src: "sfx_drink.wav",                vol: 0.65 }, // was a bit quiet
    attack:   { src: "sfx_attack.wav",               vol: 0.42 }, // was too loud
    damage:   { src: "sfx_damage.wav",               vol: 0.65 },
    player_death:{ src: "sfx_player_death.wav",       vol: 0.90 },
    game_start: { src: "sfx_game_start.wav",        vol: 0.75 },
    death_monster:{ src:"sfx_death_monster.wav",     vol: 0.60 },

    // Doors / transitions
    door_open:     { src: "sfx_door_open.wav",       vol: 0.55 },
    door_break:    { src: "sfx_door_break.wav",      vol: 0.75 },
    door_secret:   { src: "sfx_door_secret.wav",     vol: 0.60 },
    door_key_open: { src: "sfx_door_key_open.wav",   vol: 0.60 },
    cave_opening:  { src: "sfx_cave_opening.wav",    vol: 0.65 },

    descend:  { src: "sfx_descend.wav",              vol: 0.60 },
    descent_secret:{ src: "sfx_descent_secret.wav",  vol: 0.70 },

    wait:     { src: "sfx_wait.wav",                 vol: 0.26 },

    // Boss
    mtn_trial:{ src: "sfx_boss_mountain_trial.wav",  vol: 1.00 },
    mtn_fall: { src: "sfx_boss_mountain_death.wav",  vol: 1.00 }
  };
  ...
  async function initSfxWebAudio(){
    try{
      ...
      sfxMasterGain.gain.value = Math.max(0, SFX_MASTER);
      sfxMasterGain.connect(sfxCtx.destination);

      const entries = Object.entries(SFX_DEFS);

      // Load each SFX independently so a single missing/corrupt file doesn't disable all SFX.
      for(const [name, def] of entries){
        try{
          const primary = resolveAsset("sfx", def.src);

          // Optional per-SFX fallback filenames (keeps older builds compatible)
          const fb = Array.isArray(def.fallback) ? def.fallback : [];
          const urlList = [primary, def.src];
          for(const f of fb){
            try{ urlList.push(resolveAsset("sfx", f)); }catch(_){ }
            urlList.push(f);
          }

          const resp = await fetchWithFallback(urlList, { cache: "no-store" });
          const arr = await resp.arrayBuffer();
          const buf = await sfxCtx.decodeAudioData(arr);
          sfxBuffers[name] = buf;
        }catch(e){
          console.warn("SFX failed to load:", name, def?.src, e);
  ...
  function playSfx(name){
    //if(uiMuted) return;
    if(window.sfxMuted) return; // separate switch (default false)


    // file:// fallback: fetch/decodeAudioData is blocked by CORS on opaque origin "null"
    // Use HTMLAudioElement which can load file:// assets (like your bgm does).
    if(location.protocol === "file:"){
      const def = SFX_DEFS[name];
      if(!def) return;

      // Support per-SFX fallbacks on file:// too (e.g., trap_trigger -> sfx_damage_trap.wav)
      const urls = [];
      try{ urls.push(resolveAsset("sfx", def.src)); }catch(_){}
      urls.push(def.src);

      const fb = Array.isArray(def.fallback) ? def.fallback : [];
      for(const f of fb){
        try{ urls.push(resolveAsset("sfx", f)); }catch(_){}
        urls.push(f);
      }

      const vol = (def?.vol ?? 0.5);
      //const targetVol = Math.max(0, Math.min(1, vol * SFX_MASTER));
      const targetVol = Math.max(0, vol * SFX_MASTER);

      // Try each URL until one loads/plays.
      (function tryPlay(i){
        ...
        try{
          ...
          a.volume = targetVol;
          a.onended = null;
          a.onerror = function(){ tryPlay(i+1); };
          a.play().catch(function(){ /* autoplay may be blocked; will work after gesture */ });
        }catch(_){
          tryPlay(i+1);
        }
      })(0);
...
// legend collapse controls
  ...
  function escapeHtml(s){
    // Defensive: allow undefined/null without breaking render paths
    s = (s === null || s === undefined) ? "" : String(s);
    const out = s.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;");
...
function weightedChoice(items, weights){
  ...
  function mountainHarnessBuffActive(){
    return Number.isFinite(window.DEBUG_START_DEPTH) && window.DEBUG_START_DEPTH >= 19;
  }

  function applyMountainHarnessBuff(){

    // --- Set player HP for testing ---
    game.hpMax = 30;
    game.hp = 30;
    ...
    for(let i=0;i<10;i++) game.inv.potions.push(POTION.HEAL);
    for(let i=0;i<4;i++) game.inv.potions.push(POTION.INVINC);
    game.inv.potions.push(POTION.INVIS);

    // --- Weapon ---
    game.atk = 42;
    game.weaponEquipped = true;
    game.weaponName = "Test Blade";

    logMsg("DEBUG: Mountain test harness active.");
  }
  ...
  game.msg = [
    ...
    { type: "lore", text: "Whether this is an ending—or only a new beginning—none but you may say." }
  ];


    window.DD_AI.rareNarrativeInflight = false;
    window.DD_AI.rareNarrativeLastAt = -9999;
    window.DD_AI.rareNarrativeMessageCount = 0;
    window.DD_AI.messageSeq = 0;
    ...
    if(mountainHarnessBuffActive()){
      applyMountainHarnessBuff();
    }

    const __startDepth = DEBUG_START_DEPTH || 1;
    loadLevel(__startDepth);
    if(__startDepth > 1){
      logMsg(`DEBUG: start depth ${__startDepth}.`);
    }
    logMsg("");
    logMsg("");
logMsg("");
    renderAll();
    try{ playSfx('game_start'); }catch(_){ }
    directorTest();
  }

  // Allow Groq Lambda AI to direct aspecs of the game
  window.DD_DIRECTOR = window.DD_DIRECTOR || {};
  window.__ddAiMessageInflight = window.__ddAiMessageInflight || new Set();
  window.DD_AI = window.DD_AI || {};
  window.DD_AI.rareNarrativeEnabled = true;
  window.DD_AI.rareNarrativeChance = 0.015;
  window.DD_AI.rareNarrativeCooldownMessages = 30;
  window.DD_AI.rareNarrativeMaxChars = 90;
  window.DD_AI.rareNarrativeInflight = false;
  window.DD_AI.rareNarrativeLastAt = -9999;
  window.DD_AI.rareNarrativeMessageCount = 0;
  window.DD_AI.rareNarrativeEndpoint = '/api/rare-lore';
  window.DD_AI.messageSeq = window.DD_AI.messageSeq || 0;

  function ddClampRareNarrativeLine(line){
    if(typeof line !== 'string') return '';
    line = line.replace(/\s+/g, ' ').trim();
    if(!line) return '';
    if(line.length > window.DD_AI.rareNarrativeMaxChars){
      line = line.slice(0, window.DD_AI.rareNarrativeMaxChars).trim();
    }
    return line;
  }

  function ddGetRareNarrativeMode(snapshot){
    if(!snapshot) return 'watchful';
    if((snapshot.hpPercent || 0) <= 0.25) return 'blood_scent';
    if((snapshot.torches || 0) <= 1) return 'dark_hunger';
    if((snapshot.depth || 0) >= 15) return 'deep_stone';
    if((snapshot.depth || 0) >= 8) return 'ancient';
    return 'watchful';
  }

  function ddRareNarrativeMessageAllowed(text, type){
    if(typeof text !== 'string' || !text.trim()) return false;
    if(type === 'prompt' || type === 'lore' || type === 'sys') return false;
  ...
  function ddBuildRareNarrativePayload(entry){
    const recentMessage = entry && typeof entry.text === 'string' ? entry.text : '';
    const type = entry && entry.type ? entry.type : 'log';
    const hp = (game && typeof game.hp === 'number') ? game.hp : 0;
    const hpMax = (game && typeof game.hpMax === 'number') ? game.hpMax : 0;
    const payload = {
      event: 'rare_lore',
      message: recentMessage,
      type,
      depth: (game && typeof game.depth === 'number') ? game.depth : 0,
      hp,
      hpMax,
      hpPercent: hpMax > 0 ? hp / hpMax : 0,
      atk: (game && typeof game.atk === 'number') ? game.atk : 0,
      gold: (game && typeof game.gold === 'number') ? game.gold : 0,
      torches: (game && game.inv && typeof game.inv.torches === 'number') ? game.inv.torches : 0,
      healingPotions: (game && game.inv && Array.isArray(game.inv.potions)) ? game.inv.potions.filter(p => p === POTION.HEAL).length : 0,
      weaponName: game && game.weaponName,
      recentMessage
    };
    payload.mode = ddGetRareNarrativeMode(payload);
    return payload;
  }

  function ddShouldTriggerRareNarrative(entry){
    if(!window.DD_AI.rareNarrativeEnabled) return false;
    if(window.DD_AI.rareNarrativeInflight) return false;
    if(!entry || !ddRareNarrativeMessageAllowed(entry.text, entry.type)) return false;
    const count = window.DD_AI.rareNarrativeMessageCount || 0;
    const lastAt = window.DD_AI.rareNarrativeLastAt || -9999;
    if((count - lastAt) < window.DD_AI.rareNarrativeCooldownMessages) return false;
    if(Math.random() > window.DD_AI.rareNarrativeChance) return false;
    return true;
  }

  function ddReplaceMessageById(messageId, nextText){
    if(!game || !Array.isArray(game.msg)) return false;
    const clean = ddClampRareNarrativeLine(nextText);
    if(!clean) return false;
    const idx = game.msg.findIndex(m => m && m.id === messageId);
    if(idx < 0) return false;
    const current = game.msg[idx];
    if(!current || current.type === 'prompt' || current.type === 'lore') return false;
    if(current.text === clean) return false;
    game.msg[idx] = Object.assign({}, current, { text: clean, aiNarrative: true });
    renderMessages();
    return true;
  }

  async function ddRequestRareNarrativeVariant(entry){
    const payload = ddBuildRareNarrativePayload(entry);
    window.DD_AI.rareNarrativeInflight = true;
    try{
      const res = await fetch(window.DD_AI.rareNarrativeEndpoint || '/api/rare-lore', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      const data = await res.json();
      const revised = ddClampRareNarrativeLine((data && (data.revisedMessage || data.rareLore || data.text || data.message)) || '');
      if(revised){
        ddReplaceMessageById(entry.id, revised);
      }
    }catch(err){
      console.error('AI RARE NARRATIVE ERROR:', err);
    }finally{
      window.DD_AI.rareNarrativeInflight = false;
    }
  }

  function ddMaybeNarrativizeMessage(entry){
    if(!ddShouldTriggerRareNarrative(entry)) return;
    window.DD_AI.rareNarrativeLastAt = window.DD_AI.rareNarrativeMessageCount || 0;
    ddRequestRareNarrativeVariant(entry);
  }

  async function directorTest(depthOverride = null) {
    try{
    const snapshot = {
      depth: depthOverride ?? game.depth,
      hp: game.hp,
      hpMax: game.hpMax,
      hpPercent: game.hpMax > 0 ? game.hp / game.hpMax : 0,
      atk: game.atk,
      gold: game.gold,
      torches: game.inv.torches,
      healingPotions: game.inv.potions.filter(p => p === POTION.HEAL).length
    };
  ...
  function shouldAiCommentaryForMessage(text, type){
    ...
    return (
      low.startsWith('descended to level') ||
      low.startsWith('opened a secret door') ||
      low.includes('secret shortcut') ||
      low.includes('shrine') ||
      low.includes('altar') ||
      low.includes('trader') ||
      low.includes('cave') ||
      low.startsWith('weapon found') ||
      low.startsWith('no key - forced the lock')
    );
  }

  function appendAiCommentaryBelow(originalText, commentary){
    if(!game || !Array.isArray(game.msg)) return;
    const trimmed = String(commentary || '').trim();
    if(!trimmed) return;
    const line = trimmed;
    if(game.msg.some(m => m && m.text === line)) return;

    const idx = game.msg.findIndex(m => m && m.text === originalText);
    const entry = { text: line, type: 'lore', ai: true };

    if(idx === -1){
      game.msg.unshift(entry);
    } else {
      game.msg.splice(idx + 1, 0, entry);
    }
  ...
  async function maybeEnrichMessage(text, type){
    if(!shouldAiCommentaryForMessage(text, type)) return;
    if(!window.__ddAiLoreByDepth) window.__ddAiLoreByDepth = Object.create(null);
    const d = (game && Number.isFinite(game.depth)) ? game.depth : 0;
    if((window.__ddAiLoreByDepth[d] || 0) >= 1) return;

    const key = `${d}|${type}|${text}`;
    if(window.__ddAiMessageInflight.has(key)) return;
    window.__ddAiMessageInflight.add(key);

    try{
      const payload = {
        message: text,
        type,
        depth: game && game.depth,
        hp: game && game.hp,
        hpMax: game && game.hpMax,
        atk: game && game.atk,
        gold: game && game.gold,
        weaponName: game && game.weaponName
      };
      ...
      if(data && data.commentary){
        ...
        appendAiCommentaryBelow(text, data.commentary);
        window.__ddAiLoreByDepth[d] = (window.__ddAiLoreByDepth[d] || 0) + 1;
      }
    }catch(err){
      console.error('AI MESSAGE ERROR:', err);
    }finally{
      window.__ddAiMessageInflight.delete(key);
    }
  }
  ...
  // ---------- NEAR-DEATH (post-defeat continuation) ----------
function placeNearDeathWeaponDrop(lvl, wpn){
  if(!lvl || !lvl.items) return;
  if(!wpn) return;
  const safeAtk = Number.isFinite(wpn.atk) ? wpn.atk : 3;
  const safeName = (wpn.name && String(wpn.name).trim()) ? wpn.name : "Recovered Steel";
  const p = lvl.player;
  const px = p.x, py = p.y;
  ...
  for(let x=2; x<MAP_W-2; x++){
    for(let y=2; y<MAP_H-2; y++){
      ...
      // Prefer out-of-sight: not currently visible
      if(lvl.visible && lvl.visible[x] && lvl.visible[x][y]) continue;
  ...
  // Fallback if everything is visible
  if(cands.length === 0){
    for(let x=2; x<MAP_W-2; x++){
      for(let y=2; y<MAP_H-2; y++){
        if(x===px && y===py) continue;
  ...
  const kk = keyXY(pick.x, pick.y);
  lvl.items.set(kk, { type: ENT.WEAPON, name: safeName, atk: safeAtk });
}

function triggerNearDeath(lvl){
  // Near-Death means: you are NOT defeated.
  game.ended = false;
  game.victory = null;
  ...
  // 2) Clear immediate danger (room/cave simplification -> clear ALL monsters on the level)
  if(lvl && lvl.monsters) lvl.monsters.clear();

  // 3) Ransack gold
  game.gold = 0;
  game.cgold = 0;

  // 4) Potions: keep 1 heal only if you had any heals; remove others
  const hadHeal = game.inv.potions.includes(POTION.HEAL);
  game.inv.potions = hadHeal ? [POTION.HEAL] : [];

  // 5) Weapon: strip to bare hands, but drop the old weapon "just out of sight"
  const lostWeapon = game.weaponEquipped ? { name: game.weaponName, atk: game.atk } : null;

  game.atk = 3;
  game.weaponName = "Bare Hands";
  game.weaponEquipped = false;

  if(lostWeapon){
    placeNearDeathWeaponDrop(lvl, lostWeapon);
  }

  // 6) Post-defeat debuffs
  game.nd = {
    staggerLeft: 8,
    slimeLeft: 11,
    staggerMsgFired: false,
    _slimeWait: false
  };

  // 7) Messages (logged newest-first)
  fireLoreOnce(
    `nd_wake_${game.depth}`,   // per-depth key (or just 'nd_wake' if you want once-per-run)
    "You come to slowly. The area is quiet. Some of your possessions are missing. Ugh."
  );
...
function endGameLose(){
  ...
  // Near-Death intercept
    try{
      const forced = (typeof shouldForceNearDeathNow === 'function' && shouldForceNearDeathNow());

      // Normal play chance (no URL params)
      const rolled = (Math.random() < NEAR_DEATH_CHANCE);

      // Optional: prevent more than once per run
      const allowed = !game.ndUsed;

      if(forced || (allowed && rolled)){
        if(typeof consumeNearDeathHarnessIfNeeded === 'function') consumeNearDeathHarnessIfNeeded();
        game.ndUsed = true; // lock it to once per run
        triggerNearDeath(getLevel(game.depth));
        return;
      }
    }catch(_){}
    ...
    game.victory = false;
    logMsg("Defeat — the depths claim another explorer.");
  ...
  function updateContextPrompt(){
    ...
    if(ctx.type === 'relic'){
      ...
      setPromptMessage('Relic — Press [Y] to claim', `relic:${ctx.id}`);
      return;
    }
    ...
    if(ctx.type === 'shrine'){
      ...
      setPromptMessage('Shrine — Press [Y] to pray', `shrine:${ctx.id}`);
      return;
    }
  }
  ...
  function formatMessageHTML(msg){
    ...
    if(type === "prompt" && text === "Relic — Press [Y] to claim"){
      return `<span class="msg-prompt-hl">Relic — Press ${span("msg-key","[Y]")} to claim</span>`;
    }
    ...
    if(m){
      const nm = m[1];
      const atk = m[2];
      return `Weapon found (ƪ): ${span("msg-weapon", nm)} (ATK ${esc(atk)}).`;
    }
...
function renderMessages(){
  ...
  function logMsg(text, type='log'){
    ...
    const entry = { text, type, id: ++window.DD_AI.messageSeq };
    game.msg.unshift(entry);
    game.msg = game.msg.slice(0, 7);
    renderMessages();
    window.DD_AI.rareNarrativeMessageCount = (window.DD_AI.rareNarrativeMessageCount || 0) + 1;
    try{ maybeEnrichMessage(text, type); }catch(_){}
    try{ ddMaybeNarrativizeMessage(entry); }catch(_){}
  }
...
function dangerLoreTick(lvl){
  ...
  // Priority order: Dragon → Elder Wyrm/Wyrm → Horror → Cyclops/Chief → Spider/Queen → Skeleton pack
  ...
  function tickEffectsOnStep(){
    if(game.effects.poison > 0){
      ...
      if(!hasInvincible()){
        ...
        logMsg(`Poison tries to bite… but invincibility holds (${game.effects.poison-1}).`);
      }
      game.effects.poison--;
    }
...
// ---------- MOUNTAIN (Titan) ----------
// Mountain + shards fight like normal monsters (generic AI/combat).
// Spawn-only feature: avoids new engine/render paths.
function makeMountainBossForDepth(d, mode){
  if(mode === "UNKILLABLE"){
    // Trial mode: effectively unkillable (no progress).
    return { glyph: ENT.MOUNTAIN, name: "Mountain", hp: 9999, maxHp: 9999, atk: 8, __mtnPhase: 0 };
  }
  // Final Mountain: doubled durability so the post-consequence phase lasts.
  const hp = (randInt(24, 34) + Math.floor(d/2)) * 2;
  const atk = randInt(6, 10) + Math.floor(d/10);
  return {
    glyph: ENT.MOUNTAIN,
    name: "Mountain",
    hp,
...
function makeMountainShardForDepth(d, mode){
  const glyph = (Math.random() < 0.5) ? ENT.MOUNTAIN_SHARD_A : ENT.MOUNTAIN_SHARD_B;

  if(mode === "UNKILLABLE"){
    return { glyph, name: "Angry Shard", hp: 9999, atk: 5 };
  }
  // Final Mountain shards: doubled durability so they do not evaporate immediately.
  const hp = (randInt(14, 22) + Math.floor(d/4)) * 2;
  const atk = randInt(4, 8) + Math.floor(d/12);
  return { glyph, name: "Angry Shard", hp, atk };
}

function __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, cx, cy, maxR){
  for(let r=1; r<=maxR; r++){
    for(let dy=-r; dy<=r; dy++){
      for(let dx=-r; dx<=r; dx++){
        ...
        if(t===TILE.DOOR_C || t===TILE.DOOR_O || t===TILE.LDOOR_C) continue;
        const k = keyXY(x,y);
        if(monsters.has(k) || (lvl.traders && lvl.traders.has(k))) continue;
        return {x,y};
      }
    }
  }
...
// Spawns a Mountain "horde" but keeps only one █. Extra "boss slots" become shards.
function spawnMountainHorde(lvl, depth, monsters, tiles, isNormal, forbidOnTrader, startPos, downPos, desiredBossCount, mode){
  const nBoss = Math.max(1, desiredBossCount|0);

  // Always anchor to descent; if downPos is missing, do not spawn Mountain.
  if(!downPos) return 0;

  // Always place near the descent room (helps encounter readability).
  // Progressive radii avoids failure if the descent room is occupied (e.g., Intercessor).
  let bossPos =
    __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, downPos.x, downPos.y, 3) ||
    __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, downPos.x, downPos.y, 6) ||
    __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, downPos.x, downPos.y, 10);

  if(!bossPos) return 0;

  monsters.set(keyXY(bossPos.x, bossPos.y), makeMountainBossForDepth(depth, mode));

  let spawned = 1;
  const shardCount = Math.max(0, nBoss - 1);
  for(let i=0; i<shardCount; i++){
    const p =
      __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, bossPos.x, bossPos.y, 2) ||
      __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, bossPos.x, bossPos.y, 4) ||
      __findOpenNear_MTN(lvl, tiles, isNormal, forbidOnTrader, monsters, bossPos.x, bossPos.y, 7);
    if(!p) break;
    monsters.set(keyXY(p.x,p.y), makeMountainShardForDepth(depth, mode));
    spawned++;
  }
  return spawned;
}
...
// --- Mountain encounter helpers (no renderer changes) ---
const MTN_GLYPH_INTACT = ENT.MOUNTAIN; // █
const MTN_GLYPH_FRACTURED = '▓';
const MTN_GLYPH_CRACKING  = '▒';
const MTN_GLYPH_FAILING   = '░';

function isMountainLikeGlyph(g){
  return g === ENT.MOUNTAIN || g === ENT.MOUNTAIN_SHARD_A || g === ENT.MOUNTAIN_SHARD_B ||
         g === MTN_GLYPH_FRACTURED || g === MTN_GLYPH_CRACKING || g === MTN_GLYPH_FAILING;
}
function isMountainShardGlyph(g){
  return g === ENT.MOUNTAIN_SHARD_A || g === ENT.MOUNTAIN_SHARD_B;
}
function isMountainBossGlyph(g){
  return g === ENT.MOUNTAIN || g === MTN_GLYPH_FRACTURED || g === MTN_GLYPH_CRACKING || g === MTN_GLYPH_FAILING;
}

function tryMountainNudge(m, lvl, monsterMap){
  const dirs = [
    {dx:1,dy:0},{dx:-1,dy:0},
    {dx:0,dy:1},{dx:0,dy:-1}
  ];
  ...
  for(const d of dirs){
    ...
    if(canMonsterMoveTo(m, nx, ny)){
      monsterMap.delete(keyXY(m.x, m.y));
      m.x = nx; m.y = ny;
      monsterMap.set(nk, m);
      return;
    }
...
function purgeMountainEntities(monsterMap){
  // Remove Mountain and its shards from a Map<key,monster>
  for(const [k,m] of monsterMap.entries()){
    if(m && (isMountainBossGlyph(m.glyph) || isMountainShardGlyph(m.glyph))){
      monsterMap.delete(k);
    }
  }
}

// Trial messaging state lives on lvl.__mtnTrial (ephemeral; no save format changes)
function getTrialState(lvl){
  if(!lvl.__mtnTrial) lvl.__mtnTrial = { hits: 0, warned: 0, completed: false };
  return lvl.__mtnTrial;
}

function maybeLogTrialWarning(lvl){
  const st = getTrialState(lvl);
  if(st.completed) return;
  st.hits++;
  if(st.hits === 1){
    playSfx("mtn_trial"); // ← ADD THIS LINE
    logMsg("The stone does not yield. You will be tested again.", "lore");
  }else if(st.hits === 2 || st.hits === 3){
    logMsg("The stone does not yield.", "lore");
  }
  // After that: silence (the flat hit line carries the implication)
}


function updateMountainFinalPhaseAndMaybeMsg(m){
  if(!m || !isMountainBossGlyph(m.glyph)) return null;
  const maxHp = (m.maxHp || m.hp || 1);
  const pct = (m.hp <= 0) ? 0 : (m.hp / maxHp);

  // Phase thresholds: 0 intact, 1 fractured, 2 cracking, 3 failing
  let phase = 0;
  if(pct <= 0.25) phase = 3;
  else if(pct <= 0.50) phase = 2;
  else if(pct <= 0.75) phase = 1;

  const prev = (typeof m.__mtnPhase === "number") ? m.__mtnPhase : 0;
  if(phase <= prev) return null;

  m.__mtnPhase = phase;
  if(phase === 1) { m.glyph = MTN_GLYPH_FRACTURED; return "Stone cracks."; }
  if(phase === 2) { m.glyph = MTN_GLYPH_CRACKING;  return "The earth recoils."; }
  if(phase === 3) { m.glyph = MTN_GLYPH_FAILING;   return "The Mountain buckles."; }
  return null;
}

function applyMountainFinalStrikeConsequences(m){
  if(!m) return;
  m.__mtnFinalHits = (m.__mtnFinalHits || 0) + 1;

  if(m.__mtnFinalHits === 1 && !m.__mtnDulled){
    m.__mtnDulled = true;
    if(game.weaponEquipped){
      game.atk = Math.max(3, Math.floor(game.atk / 2));
      if(game.weaponName && !/\(dulled\)$/i.test(game.weaponName)){
        game.weaponName += " (dulled)";
      }
    }
    logMsg("Your blade dulls.", "lore");
    return;
  }

  if(m.__mtnFinalHits === 2 && !m.__mtnGlass){
    m.__mtnGlass = true;
    let lost = 0;
    game.inv.potions = game.inv.potions.filter(p => {
      if(p === POTION.INVINC){
        lost++;
        return false;
      }
    ...
    logMsg("Glass shatters.", "lore");
    if(lost > 0){
      logMsg("Your invincibility potions are gone.", "lore");
    }
  }
}
...
function spawnSkeletonPacks(lvl, depth, monsters, tiles, isNormal, forbidOnTrader, startPos){
  // Skeletons do NOT spawn as singles. Packs of 2–5, moving as a herd.
  if (depth < 1) return 0;
  ...
  for(let pack=0; pack<maxPacks; pack++){
    ...
    const n = Math.min(randInt(2,5), remaining); // pack size 2–5 (clamped by remaining cap)
 ...
 * - Encounter awakens only if player steps onto the Intercessor ('╬') or a Supplicant ('^') overlay.
...
function intercessorStageCheckOnPlayerStep(lvl, prevX, prevY, px, py){
  ...
  if(inFieldNow && !inFieldPrev && !st.loreFired){
    st.loreFired = true;
    fireLoreOnce('lore_intercessor', 'The air tightens. A rite unfolds — unbroken.');
  }
...
function awakenIntercessorStage(lvl){
  ...
  fireLoreOnce('lore_intercessor_disturb', 'You have no idea what you’ve done.');
...
// Boss glyph: Φ (U+03A6), minion glyph: ⊗ (U+2297)
...
function awakenAmbushStage(lvl){
  ...
  // Minions ⊗: 5–7, mix of wardens + compressors
  ...
  // It is placed in 1–2 rooms every 2–3 levels, and never in start/end rooms.
  const BAD_FURN = {
    throne: {
      type: 'throne',
      anchors: ['wall'],
      variants: [
        { w:2, h:3, rows: ["┌ ", "║█", "└ "] },       // minimal workbench throne
        { w:3, h:3, rows: ["╔═╦", "║█╬", "╚═╩"] }     // ceremonial throne
      ]
    },
    library: {
      type: 'library',
      anchors: ['wall'],
      // Modular shelving: 1–3 bays. We build the stamp on demand.
      build: function(bays){
        ...
        const top = "╔" + ("═══╦".repeat(bays-1)) + "═══╗";
        const mid = "║" + ("█ █ ".repeat(bays)).slice(0, bays*4-1) + "║";
        const bot = "╚" + ("═══╩".repeat(bays-1)) + "═══╝";
        ...
        if(midFixed.length > need) midFixed = midFixed.slice(0, need-1) + "║";
  ...
  function placeBADFurniture(lvl, rooms, depth, startPos, downPos){
    if(!rooms || rooms.length === 0) return;
    // Frequency: 1–2 furniture rooms every 2–3 levels (low touch)
    if(depth < 1) return;
...
// ===== BAD STATUES (Ω Witness galleries; inert stone; lore-only) =====
function placeBADStatues(lvl, rooms, depth, startPos, downPos){
  ...
  // Sparse otherwise: roughly every 4–6 levels
  if(!force){
    if(depth < 2) return;
  ...
  if(eligible.length===0){
    if(!force) return;
    // For forced test (depth 1), allow start/end rooms as a fallback so you always see Ω.
...
// ===== END BAD STATUES =====
  ...
  function generateLevel(depth){
      ...
      const rooms = [];
      const dd = window.DD_DIRECTOR || {};

      const aiWantsMicro = !!dd.microLevel;
      const aiForceStartShrine = !!dd.forceStartShrine;
      const aiExtraHealPotions = Math.max(0, Math.min(3, Math.floor(Number(dd.extraHealPotions) || 0)));
      const aiExtraTorches = Math.max(0, Math.min(3, Math.floor(Number(dd.extraTorches) || 0)));
      const aiTraderBias = Math.max(1, Math.min(3, Number(dd.traderBias) || 1));
      const aiMonsterBias = Math.max(0.55, Math.min(1.2, Number(dd.monsterBias) || 1));

      const roomTarget = aiWantsMicro
        ? randInt(2, 4)
        : randInt(ROOMS_MIN, ROOMS_MAX);

      console.log("DD_DIRECTOR raw:", dd);
      console.log("DD_DIRECTOR applied:", {
        depth: depth,
        microLevel: aiWantsMicro,
        roomTarget: roomTarget,
        forceStartShrine: aiForceStartShrine,
        extraHealPotions: aiExtraHealPotions,
        extraTorches: aiExtraTorches,
        traderBias: aiTraderBias,
        monsterBias: aiMonsterBias
      });
      ...
      if(!aiWantsMicro && rooms.length < 5){
      ...
      // Embedded cave systems (0–2 per level) live below the dungeon in the extra map band.
  ...
  chooseBlackoutRooms(lvl, rooms, depth, startPos, downPos);
      ...
      // Ambush stage: choose 0–1 rooms per level (latent until triggered).
      ...
      if(chance(Math.min(0.9, TRADER_SPAWN_CHANCE * aiTraderBias))){
      ...
      if((DEBUG && DEBUG_FORCE_VITALITY_SHRINE) || aiForceStartShrine){
      ...
      const torchCount =
        TORCH_BASE_COUNT +
        Math.floor(depth / TORCH_BONUS_EVERY_LEVELS) * TORCH_PER_LEVEL_BONUS +
        aiExtraTorches;
      ...
      const potionCount = randInt(3,5) + Math.floor(depth/5) + aiExtraHealPotions;
      ...
      let monsterCount = Math.max(
        0,
        Math.round((randInt(3,6) + Math.floor(depth/3)) * aiMonsterBias)
      );
  ...
  // Normal gold-only chests (key chests added later)
      ...
      // Mountain (Titan) encounter:
      // Real play: depth 10 = TRIAL, depth 20 = FINAL
      // Debug harness: depth 1 can impersonate TRIAL or FINAL via window.DEBUG_MOUNTAIN_MODE
      const __mMode = getMountainMode(depth);
      if(__mMode){
        lvl.__mtnMode = __mMode;
        const __spawnMode = (__mMode === "TRIAL") ? "UNKILLABLE" : "DEFEATABLE";
        spawnMountainHorde(lvl, depth, monsters, tiles, isNormal, forbidOnTrader, startPos, downPos, randInt(2,4), __spawnMode);
      }

      // Steel encounter harness + real encounters (depth 4 and 13)
      const __sMode = getSteelMode(depth);
      if(__sMode){
        spawnSteelEncounter(lvl, depth, __sMode);
      }

      // Boss on 20th level: Deep Horror
      if(depth === 20){      
      ...
      // Step 3c: place BAD statues (Ω witness formations; lore-only)
      if (typeof placeBADStatues === 'function') placeBADStatues(lvl, rooms, depth, startPos, downPos);

      // TEST HARNESS: ensure Ω statues are visible early for validation (Lvls 1–3)
      /*
      if(depth <= 3){
        try{
...
// Spawn cave-only Scroll of Illumination (☼): auto-read on pickup.
...
function spawnCaveIlluminationScrolls(lvl, depth){
  ...
  scatter(lvl.items, tiles, count, () => ({ type: "☼", caveIllum: true }), canPlaceCave, null);
}
...
function generateCaves(lvl, depth){
    // Carve 0–2 cave systems per level into the extra "band" below the dungeon.
...
// ---------- TRADER PROMPT DEFAULT N ----------
  ...
  function resolveChaosRelicAt(x, y){
    ...
    lvl.items.delete(k);
    playSfx("pickup_relic");
    logMsg("The relic shatters — something answers the call.");
    fireLoreOnce("cave_relic_shatter", "The old stone learns your name.");
    startTileAnim(x, y, "monster_death", 700, { glyph: 'ѧ', color: COL.chaosRelic });
...
function roomPassableInteriorCount(lvl, r){
  ...
  function playerAttack(lvl, mx, my){
    ...
    let k = keyXY(mx,my);
    let m = lvl.monsters.get(k);
    const __mMode = getMountainMode(game.depth);
    
    // Mountain TRIAL: no progress; hits only teach.
    if(m && __mMode === "TRIAL" && (isMountainBossGlyph(m.glyph) || isMountainShardGlyph(m.glyph))){
      maybeLogTrialWarning(lvl);
      logMsg(isMountainBossGlyph(m.glyph) ? "You hit Mountain." : "You hit a shard.");

      // Allow pressure to build even in TRIAL
      if(isMountainBossGlyph(m.glyph)){
        m.__hitCount = (m.__hitCount || 0) + 1;
        if(m.__hitCount % 5 === 0){
          tryMountainNudge(m, lvl, next);
        }
      }
      return;
    }
    ...
    // Spider SHELL proxy: Queen cannot be hit while her shell is intact.
    // While at least one spider guard remains adjacent to the Queen, your strike is diverted into the shell.
    if(m && m.packId && m.packLeader && m.herdType === "spider"){
      const packId = m.packId;
      const guards = [];
      for(const [kk, mm] of lvl.monsters.entries()){
        if(!mm || mm.packId !== packId) continue;
        if(mm.packLeader) continue;
        if(mm.herdType !== "spider") continue;
        const pos = parseKey(kk);
        const distQ = Math.max(Math.abs(pos.x - mx), Math.abs(pos.y - my));
        if(distQ <= 1){
          const distP = Math.max(Math.abs(pos.x - _p.x), Math.abs(pos.y - _p.y));
          guards.push({ kk, mm, x: pos.x, y: pos.y, distP });
        }
      }
      if(guards.length){
        guards.sort((a,b)=>a.distP - b.distP);
        const g = guards[0];
        logMsg("The Spider Queen is shielded by her shell.");
        k = g.kk; m = g.mm; mx = g.x; my = g.y;
      }
    }
    ...
    if(__mMode === "FINAL" && m && isMountainBossGlyph(m.glyph)){
      applyMountainFinalStrikeConsequences(m);
    }

    // --- Steel encounters: sardonic stage messaging + immobile feel ---
    if(m && m.steelKind){
      if(!m.__steelIntro){
        m.__steelIntro = true;
        if(m.steelKind === "BRO"){
          ...
          logMsg("Excuse me—my sharper edges… but they cut!", "lore");
        }
      }
      if(m.hp > 0 && m.steelKind === "HUNK"){
        const maxHp = (m.hpMax || 1);
        const pct = (m.hp / maxHp);
        let stage = 0;
        if(pct <= 0.25) stage = 3;
        else if(pct <= 0.50) stage = 2;
        else if(pct <= 0.75) stage = 1;
        if(typeof m.steelStage !== "number") m.steelStage = 0;
        if(stage > m.steelStage){
          m.steelStage = stage;
          if(stage === 1) logMsg("That thing is aware?", "lore");
          if(stage === 2) logMsg("A clatter spreads.", "lore");
          if(stage === 3) logMsg("The clanking mass seems to wince.", "lore");
        }
      }
    }

    // --- Mountain anti-spam nudge (player-driven) ---
    if(isMountainBossGlyph(m.glyph)){
    const dist = Math.max(
        Math.abs(_p.x - m.x),
        Math.abs(_p.y - m.y)
    ...
    if(dist !== 1){
        // adjacency broken → reset pressure
        m.__hitCount = 0;
    } else {
        m.__hitCount = (m.__hitCount || 0) + 1;

        // Every 5 consecutive adjacent hits, Mountain settles
        if(m.__hitCount % 5 === 0){
        tryMountainNudge(m, lvl, next);
        }
    }
    ...
    if(__mMode === "FINAL" && m && isMountainBossGlyph(m.glyph) && m.hp > 0){
      const __phaseMsg = updateMountainFinalPhaseAndMaybeMsg(m);
      if(__phaseMsg) logMsg(__phaseMsg, "lore");
    }
    if(m.hp <= 0){
      if(__mMode === "FINAL" && m && isMountainBossGlyph(m.glyph)){
        // Final payoff: ceremonial treasure.
        try{ game.gold += 1000; }catch(_){ }
        logMsg("The Mountain breaks.", "lore");
        logMsg("You gain +1000 gold.");
        playSfx("mtn_fall");
      }
      if(m && m.steelKind){
        if(m.steelKind === "BRO") logMsg("The older mass settles, then hardens.", "lore");
        else logMsg("The hunk sulks, then seeps away, eyeballs behind.", "lore");
      }
  ...
  try{
    ...
    setTimeout(() => { try{ renderScreen(); }catch(_e){} }, 0);
      ...
      // Ambush resolution: if the Authority Node (Φ) falls, remaining bound agents (⊗) dissolve.
      if(m && m.isAmbushBoss){
        const st = lvl && lvl.ambushStage;
        if(st){ st.resolved = true; }
        for(const [kk, mm] of lvl.monsters.entries()){
      ...
      if(m && m.isIntercessor) bounty = bounty * 3; // Intercessor pays out big (2–3x typical)
      // Arbiter-only payout (ambush boss): fixed ceremonial range
      if(m && m.isAmbushBoss) bounty = randInt(20, 50);
      // Steel bounty enhancement (fixed): Hunk=20, Bro=50
      if(m && m.steelKind) bounty = (m.steelKind === "BRO") ? 50 : 20;
      game.gold += bounty;
  ...
  function tryMoveMonster(k, m, x, y, dx, dy){
    const nx = x + dx, ny = y + dy;
    const nk = keyXY(nx,ny);
  ...
  function isCyclopsTempo(m){
    return m && m.herdType === "cyclops";
  }
  function tempoGateOrConsume(m){
    if(!isCyclopsTempo(m)) return false;     // not gated
    if(!m._tempoWait){ return false; }       // not waiting
    m._tempoWait = false;                    // consume wait
    return true;                             // skip this turn
  }
  function tempoMarkUsed(m){
    if(isCyclopsTempo(m)) m._tempoWait = true;
  }
...
if(m && m.immobile) return null;
  ...
  // Ambush minions (⊗): wardens guard exits; compressors converge toward player.
...
if(m && m.guardIntercessor){
  ...
  for(const [k, m] of lvl.monsters.entries()){
    ...
    // immobile monsters don't move, but they CAN still attack if adjacent
    ...
    // Mountain TRIAL
    if(isMountainBossGlyph(m.glyph) && dist > 1){
        m.__hitCount = 0;
    }
    ...
    if(dist === 1){
      ...
      // Cyclops TEMPO proxy: after acting, cyclops must recover for 1 turn.
      if(tempoGateOrConsume(m)) continue;
      // Cyclops TEMPO proxy: after acting, cyclops must recover for 1 turn.
      if(tempoGateOrConsume(m)) continue;
      const dmg = randInt(1, m.atk);

      // Mountain TRIAL: drain to 1 HP, then withdraw (no death).
      if(__mMode === "TRIAL" && (isMountainBossGlyph(m.glyph) || isMountainShardGlyph(m.glyph)) && !hasInvincible()){
        const newHp = game.hp - dmg;
        game.hp = Math.max(1, newHp);
        playSfx('damage');
        logMsg(`${m.name} hits for ${dmg}.`);
        tempoMarkUsed(m);
        tempoMarkUsed(m);
        if(game.hp <= 1){
          // Trial ends: Mountain and shards vanish.
          try{ getTrialState(lvl).completed = true; }catch(_){}
          purgeMountainEntities(next);
          lvl.monsters = next;
          logMsg("You were warned. The test is not complete.", "lore");
          return;
        }
        continue;
      }
      ...
      if(hasInvincible()){
        logMsg(`${m.name} strikes — but it cannot harm you.`);
        tempoMarkUsed(m);
        tempoMarkUsed(m);
      } else {
        game.hp -= dmg;
    ...
    // After resolving adjacency attacks, immobile monsters do not take movement actions.
    if(m && m.immobile) continue;


    // Cyclops TEMPO proxy: if waiting, skip this turn.
    if(tempoGateOrConsume(m)) continue;

    const step = computeChaseOrWander(x,y,m);
    if(!step) continue;

    const nk = tryMoveMonster(k, m, x, y, step.dx, step.dy);
    if(nk !== k) tempoMarkUsed(m);
...
if(m && m.herdType === "spider"){
  // Firm shell: once a spider is in the Queen's ring, it holds position.
  if(distL <= 1) continue;
} else if(distL <= 2){
...
if(dx===0 && dy===0) continue;
    ...
    for(const a of attempted){
      ...
      const nk = tryMoveMonster(k, m, x, y, a.dx, a.dy);
      if(nk !== k || next.size !== beforeSize){
        moved = (nk !== k);
        if(moved) tempoMarkUsed(m);
        break;
      }
    }
    if(!moved){
    ...
    if(t === TILE.LDOOR_C){
      ...
      if(game.inv.keys <= 0){
        if(LOCKED_DOOR_BREAKS_WITHOUT_KEY){
          ...
          playSfx("door_break");
          if(hasInvincible()){
            logMsg("No key — forced the lock. It shatters harmlessly.");
          } else if(LOCK_BREAK_DAMAGE > 0){
            game.hp -= LOCK_BREAK_DAMAGE;
            logMsg(`No key — forced the lock. Door breaks open! (-${LOCK_BREAK_DAMAGE} HP)`);
            if(game.hp <= 0){
              game.hp = 0;
              endGameLose();
              return true;
            ...
            logMsg("No key — forced the lock. Door breaks open!");
          }
        } else {
...
// --- Near-Death movement effects (PLAYER) ---
// Stagger: random 8-way direction for 3 movement attempts (message once)
// Slime: consume every other movement attempt for 6 attempts (silent)
if(game.nd && (dx !== 0 || dy !== 0)){
  if(game.nd.staggerLeft > 0){
    if(!game.nd.staggerMsgFired){
      logMsg("Your senses betray you.");
      game.nd.staggerMsgFired = true;
    }
    const dirs = [
      [-1,-1],[0,-1],[1,-1],
      [-1, 0],       [1, 0],
    ...
    const pick = dirs[randInt(0, dirs.length - 1)];
    dx = pick[0];
    dy = pick[1];
    game.nd.staggerLeft--;
  }

  if(game.nd.slimeLeft > 0){
    if(!game.nd._slimeWait){
      game.nd._slimeWait = true;
      game.nd.slimeLeft--;
      ...
      game.nd._slimeWait = false;
      game.nd.slimeLeft--;
    }

    if(game.nd.slimeLeft <= 0 && game.nd.staggerLeft <= 0){
      game.nd = null;
    }
  }
}
    ...
    if(lvl.monsters.has(mk)){
      ...
      if(isBridgeAny(lvl, p.x, p.y) && isBridgeAny(lvl, nx, ny)){
        ...
        if(_m && !_m.pacified){
          ...
          if(hasInvincible()){
            logMsg(`${_m.name} strikes — but it cannot harm you.`);
          } else {
            game.hp -= _dmg;
            playSfx('damage');
    ...
    if(steppingOnCorpse){
      if(!game.corpseWait){
        ...
        // Less conservative: count corpses in a 5×5 area (radius 2) around the step target.
        let local = 0;
        ...
        if(!game.corpseFieldLoreShown){
            logMsg("You slog slowly through the corpse field, surrounded by the stench — and the satisfaction — of what you left behind.");
...
if(!__wasInBlackout && __nowInBlackout){
  ...
  logMsg("Not even your torch dispels the darkness lurking here.");
    ...
    const __t = lvl.tiles[p.x][p.y];
    if(__t === TILE.CORPSE) playSfx('step_corpsefield');
    else playSfx('step');
    ...
    if(lvl.traps.has(mk)){
      ...
      if(hasInvincible()){
        logMsg("A trap triggers — but you are invincible.");
    ...
    if(lvl.items.has(mk)){
      ...
      if(it.type === ENT.POTION){
        ...
        playSfx('pickup_scroll');
        const nm = SCROLL_NAME[it.type] || 'Mystery';
        if(it.type === "☼"){ // Illumination
          ...
          logMsg(`You unfurl the scroll of ${nm}… its magic is not yet bound.`);
        }
        lvl.items.delete(mk);
      }
        ...
        playSfx('pickup_chest');
        lvl.items.delete(mk);
      } else if(it.type === ENT.WEAPON){
        // Defensive: weapon items may be malformed (e.g., post-defeat drops). Never let that break rendering.
        const safeAtk = Number.isFinite(it.atk) ? it.atk : 3;
        const safeName = (it.name && String(it.name).trim()) ? it.name : "Unknown Steel";

        game.atk = Math.max(game.atk, safeAtk);
        game.weaponName = safeName;
        playSfx('pickup_weapon');
...
if(_hadWeapon) game.weaponUpgradeCount++;
        ...
        logMsg(`Weapon found (ƪ): ${safeName} (ATK ${safeAtk}).`);
        lvl.items.delete(mk);
  ...
  async function tryDescend(){
    ...
    // ask AI about the floor we are about to generate
    await directorTest(nextDepth);
  ...
  function drinkPotionType(type){
    ...
    logMsg("Drank a strange potion… nothing happens.");
    renderAll();
  }
  ...
  function spanChar(ch, color, dim=false, mx=null, my=null, meta=null){
    // Glyph-only override: show skull icon in canvas mode, ASCII-safe § in DOM mode
    if(!window.SAFE_MODE && ch === ENT.SKELETON) { ch = '☠'; }
...
const safe = ch === ' ' ? '&nbsp;' : escapeHtml(ch);
  ...
  function baseColorFor(lvl, x, y, glyph){
    if(glyph === '+' || glyph === '–' || glyph === '|') return COL.wall;
    const t = lvl.tiles[x][y];
      ...
      for(let x=vx0; x<vx0+VIEW_W; x++){
        ...
        if(vis){
          if(lvl.items.has(k)){
            ...
            // Chaos Relic: logic 'A', render glyph 'ѧ'
            if(it.type === 'A'){
              glyph = 'ѧ';
